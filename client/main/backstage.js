...
const BackstageModel = Backbone.Model.extend({
    ...
    initialize : function(args) {
        ...
        ActionHub.get('save').on('request', () => this.requestDefaultSave());
    ...
    setCurrentDirectory: function(wdType, filePath, type) {
        if (filePath === '')
            filePath = this._wdData[wdType].defaultPath;

        if (wdType === 'examples' && filePath.startsWith('{{Examples}}') === false)
            filePath = this._wdData[wdType].defaultPath;
        ...
        let promise = this.instance.browse(filePath);
    ...
    async requestDefaultSave() {

        if (this.instance.get('saveFormat')) {
            try {
                this.setSavingState(true);
            ...
            await this.requestSave(this.instance.get('path'), true);
        }
    },
